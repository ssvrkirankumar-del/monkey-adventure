Shader "Squirrel/Fur Card HDRP"
{
    ...
    SubShader
    {
        ...
        // Editor selection outline mask — see FurShell_HDRP: the outline system needs a clip
        // WITHOUT the alpha dither, or every stipple hole reads as a mask edge and the whole
        // coat speckles while selected.
        Pass
        {
            ...
            HLSLPROGRAM
            #pragma target 3.5
            #pragma vertex SelectionVert
            #pragma fragment SelectionFrag
            ...
            CardDepthVaryings SelectionVert(CardAttributes IN)
            {
                CardDepthVaryings o = (CardDepthVaryings)0;
                float3 posOS = CardScaleAndApplyCombAndGravity(IN.positionOS.xyz, IN.normalOS, IN.tangentOS, IN.uv1, IN.uv, IN.controlUV.xy, IN.baseUV);
                float3 positionWS = TransformObjectToWorld(posOS);
            ...
            float4 SelectionFrag(CardDepthVaryings IN) : SV_Target
            {
                clip(CardAlpha(IN.uv, IN.controlUV) - _Cutoff);   // no dither — clean mask
